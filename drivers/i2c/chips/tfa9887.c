...
int dsp_read_mem(struct tfa9887_t *tfa9887, unsigned short start_offset,
...
int dsp_set_param(struct tfa9887_t *tfa9887, unsigned char module_id,
		unsigned char param_id, const unsigned char *data, int num_bytes)
{
	...
	if (error == Tfa9887_Error_Ok) {
		if (rpcStatus != STATUS_OK) {
			error = rpcStatus+100;
			TFA_LOGE("%s %s: RPC rpcStatus =%d\n", TAG, __func__, error);
		}
	}
	return error;
}

int dsp_get_param(struct tfa9887_t *tfa9887, unsigned char module_id,
...
static int tfa9887_enable_dsp(struct tfa9887_t *tfa9887, int enable)
{
...
static int tfa9887_select_input(struct tfa9887_t *tfa9887, int input)
{
	...
	/* TODO: how to handle tfa9887L/tfa9887R */
	switch (input) {
		case 1:
			value |= 0x40;
			break;
...
int tfa9887_select_channel(struct tfa9887_t *tfa9887, int channels)
{
	int error;
	unsigned int value;
...
int tfa9887_set_sample_rate(struct tfa9887_t *tfa9887, int sample_rate)
{
	int error;
	unsigned int value;
...
int tfa9887_wait_ready(struct tfa9887_t *tfa9887)
{
	int error;
	unsigned int value;
	int ready, tries;
...
int tfa9887_startup(struct tfa9887_t *tfa9887)
{
	int error;
	unsigned int value;
	...
	if (Tfa9887_Error_Ok == error) {
		/*  adjusted to mandatory defaults */
...
int Tfa9887_Init(int sample_rate)
{
	...
	error = tfa9887_set_sample_rate(tfa9887, 48000);
	if (error != Tfa9887_Error_Ok) {
		TFA_LOGE("Unable to set sample rate\n");
		goto pub_init_err;
	}
	error = tfa9887_select_channel(tfa9887, 0);
	if (error != Tfa9887_Error_Ok) {
	...
	error = tfa9887_power(tfa9887, 0);
	if (error != Tfa9887_Error_Ok) {
		TFA_LOGE("Unable to powerdown\n");
		goto pub_init_err;
	}

	/* wait for ready */
	error = tfa9887_wait_ready(tfa9887);
	if (error != Tfa9887_Error_Ok) {
...
static bool spkamp_prev_enabled = false;
void Tfa9887_SetSpkAmp(int enable)
{
	struct tfa9887_t *tfa9887 = NULL;
	int error;
	...
	if (enable && !spkamp_prev_enabled) {
		/* off->on transition */
		spkamp_prev_enabled = true;
	...
	} else if (!enable && spkamp_prev_enabled) {
		/* on->off transition */
		spkamp_prev_enabled = false;
		if (tfa9887->dsp_enabled) {
			error = tfa9887_power(tfa9887, true);
...
EXPORT_SYMBOL(Tfa9887_SetSpkAmp);
		...
		const struct i2c_device_id *id)
{
	...
	TFA_LOGI("%s %s: tfa9887R_i2c_probe\n", TAG, __func__);
...
static int __init tfa9887_modinit(void)
{
	...
	if (ret != 0) {
		TFA_LOGE("%s %s: Failed to register tfa9887 I2C driver: %d\n",
				TAG, __func__, ret);
	}
